...
class PublishGuideServiceProvider extends ServiceProvider {
	...
	public function boot() {
		...
		$this->maybe_redirect_to_site_design_controls();
		$this->maybe_redirect_to_add_domain();
		$this->maybe_redirect_to_seo_settings();
		...
		add_action(
			...
			function( $hook_suffix ) use ( $build_file_slug, $asset_file, $enqueue_handle ) {
				...
				if ( ! is_admin() && false !== $edit_post_script_index ) {
				...
				wp_localize_script(
					...
					(array) apply_filters(
						...
						array(
							...
							'stylesRedirectUrl' => $this->styles_redirect_url(),
	...
	public function is_excluded_admin_page() {
		...
		if ( \is_plugin_active( 'divi-builder/divi-builder.php' ) ) {
			$excluded_pages = array_merge(
				...
				array(
					...
					'et_divi_role_editor',
					'et_divi_library',
					'et_divi_support_center',
					'et_divi_theme_builder',
					'et_divi_dashboard',
					'et_divi_cloud',
					'et_divi_layouts',
					'et_divi_error_report',
				)
			);
			$excluded_types = array_merge(
				$excluded_types,
		...
		return $this->is_excluded_page( $excluded_pages ) || $this->is_excluded_post_type( $excluded_types );
	}

	/**
	 * Determines if the current page is among the excluded pages.
	 *
	 * @param array $excluded_pages List of page identifiers to be excluded.
	 *
	 * @return bool
	 */
	private function is_excluded_page( $excluded_pages ) {
		global $pagenow;
		$page_arg = empty( $_GET['page'] ) ? '' : sanitize_key( $_GET['page'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return in_array( $pagenow, array( 'admin.php' ), true ) && in_array( $page_arg, $excluded_pages, true );
	}

	/**
	 * Determines if the current post type is in the list of excluded types.
	 *
	 * @param array $excluded_types Array of post types to exclude.
	 *
	 * @return bool
	 */
	private function is_excluded_post_type( $excluded_types ) {
		global $pagenow;
		...
		return in_array( $pagenow, array( 'edit.php', 'post-new.php', 'edit-tags.php', 'term.php' ), true ) && in_array( $post_type_arg, $excluded_types, true );
	}
	...
	/**
	 * Redirect to the styles editor.
	 *
	 * @return string
	 */
	private function styles_redirect_url() {
		// Block editor theme.
		...
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			if ( ! empty( $page_on_front ) ) {
				return add_query_arg(
					...
					get_edit_post_link( $page_on_front, 'edit' )
				);
			}
		}
	...
	/**
	 * Redirect the user to the site design controls if they are trying to edit styles.
	 * Note: Can be access via URL query param `gdl_action=edit-styles`.
	 */
	protected function maybe_redirect_to_site_design_controls() {
		$gdl_action = filter_input( INPUT_GET, 'gdl_action', FILTER_DEFAULT );

		if ( ! $gdl_action || 'edit-styles' !== sanitize_text_field( $gdl_action ) ) {
			return;
		}

		wp_safe_redirect( $this->editor_redirect_url() . '&tooltip=siteDesign&adminClick=true' );
		exit;
	}

	/**
	 * Redirect the user to the add domain screen in myh.
	 * Note: Can be access via URL query param `gdl_action=add-domain`.
	 */
	protected function maybe_redirect_to_add_domain() {
		$gdl_action = filter_input( INPUT_GET, 'gdl_action', FILTER_DEFAULT );

		if ( ! $gdl_action || 'add-domain' !== sanitize_text_field( $gdl_action ) ) {
			return;
		}

		// Allow safe redirects to host.godaddy.com.
		add_filter(
		...
		wp_safe_redirect( $this->get_change_domain_uri() );
		exit;
	}

	/**
	 * Redirect the user to the SEO settings.
	 * Note: Can be access via URL query param `gdl_action=setup-seo`.
	 */
	protected function maybe_redirect_to_seo_settings() {
		$gdl_action = filter_input( INPUT_GET, 'gdl_action', FILTER_DEFAULT );

		if ( ! $gdl_action || 'setup-seo' !== sanitize_text_field( $gdl_action ) ) {
			return;
		}

		$is_yoast_installed         = file_exists( WP_PLUGIN_DIR . '/wordpress-seo/wp-seo.php' );
		$is_yoast_premium_installed = file_exists( WP_PLUGIN_DIR . '/wordpress-seo-premium/wp-seo-premium.php' );

		$is_yoast_installed_and_active         = is_plugin_active( 'wordpress-seo/wp-seo.php' );
		$is_yoast_premium_installed_and_active = is_plugin_active( 'wordpress-seo-premium/wp-seo-premium.php' );

		// Neither is installed. Install Yoast SEO.
		if ( ! $is_yoast_installed ) {
			...
			if ( $install ) {
				$activate_yoast = activate_plugin( 'wordpress-seo/wp-seo.php' );

				if ( is_wp_error( $activate_yoast ) ) {
					return;
				}
			}
		}

		// Yoast is installed and not active. (activate Yoast SEO).
		if ( $is_yoast_installed && ! $is_yoast_installed_and_active ) {
			$activate_yoast = activate_plugin( 'wordpress-seo/wp-seo.php' );
		...
		// Yoast Premium is installed and not active. (activate Yoast SEO Premium).
		if ( $is_yoast_premium_installed && ! $is_yoast_premium_installed_and_active ) {
			$activate_yoast_premium = activate_plugin( 'wordpress-seo-premium/wp-seo-premium.php' );

			if ( is_wp_error( $activate_yoast_premium ) ) {
				return;
			}
		}

		wp_safe_redirect( $this->yoast_seo_redirect_url() );
		exit;
	}
