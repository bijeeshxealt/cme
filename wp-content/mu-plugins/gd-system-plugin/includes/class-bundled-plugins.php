...
final class Bundled_Plugins {
	...
	public function __construct() {
		...
		$bundled_plugins = [
			...
			'automattic/wordpress-mcp/wordpress-mcp.php' => $this->should_load_mcp_plugin(),
	...
	/**
	 * Check if the MCP plugin should be loaded.
	 * Note: The MCP plugin is only loaded under the following conditions:
	 * - The feature flag 'wordpress_mcp' is enabled.
	 * - The plugin is not already installed.
	 * - GD_DISABLE_MCP is not defined or set to false.
	 *
	 * @return bool
	 */
	private function should_load_mcp_plugin() {

		// Check if the feature flag is enabled for MCP.
		if ( ! $GLOBALS['wpaas_feature_flag']->get_feature_flag_value( 'wordpress_mcp', false ) ) {
			return false;
		}
		...
		// Check if GD_DISABLE_MCP is defined and set to true.
		if ( defined( 'GD_DISABLE_MCP' ) && GD_DISABLE_MCP ) {
			return false;
		}
